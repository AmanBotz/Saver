...
Y = None if not STRING else import('shared_client').userbot
Z, P, UB, UC, emp = {}, {}, {}, {}, {}
...
await C.edit_message_text(h, m, f"Pyro Handler...\n\n{bar}\n\n⚡Completed: {c_mb:.2f} MB / {t_mb:.2f} MB\n📊 Done: {p:.2f}%\n🚀 Speed: {speed:.2f} MB/s\n⏳ ETA: {eta}\n\n**Powered by Team SPY**")
...
if m.media:  
        ...
        p = await c.send_message(d, 'Downloading...')  
            ...
            if not orig_filename:
                orig_filename = f"msg_{int(time.time())}"

            safe_filename = re.sub(r'[<>:"/\\|?*]', '_', orig_filename)

            download_dir = os.path.join('downloads', f"user_{uid}")
